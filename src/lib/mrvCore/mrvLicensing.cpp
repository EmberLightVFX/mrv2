...
namespace
{
    ...
    const std::string verify_key_b64 =
        "V/7Jp4Ngx1g1sSiovQgxl9utH4oylriTMZFNNLHefGU=";
} // namespace
...
namespace
{
    ...
    bool verify_ed25519(
        const std::string& pubkey_b64, const std::string& message,
        const std::string& signature_b64)
    {
        ...
        {
            LOG_ERROR(
                "Error: Invalid public key length. Expected 32, got "
                << pubkey_bytes.size() << ".");
            return false;
        }
        ...
        EVP_PKEY* pkey = EVP_PKEY_new_raw_public_key(
            EVP_PKEY_ED25519, nullptr, pubkey_bytes.data(),
            pubkey_bytes.size());
        if (!pkey)
        {
        ...
        {
            LOG_ERROR(
                "OpenSSL Error: Failed to initialize digest verification.");
            ERR_print_errors_fp(stderr);
            EVP_MD_CTX_free(ctx);
            EVP_PKEY_free(pkey);
            return false;
        ...
        int result = EVP_DigestVerify(
            ctx, sig_bytes.data(), sig_bytes.size(),
    ...
    void activatePlan(const std::string& plan)
    {
        ...
        LOG_STATUS(msg);
        msg = tl::string::Format(_("Supports annotations '{0}'"))
                  .arg(mrv::app::soporta_annotations);
        LOG_INFO(msg);
        msg = tl::string::Format(_("Supports editing '{0}'"))
                  .arg(mrv::app::soporta_editing);
        LOG_INFO(msg);
        msg = tl::string::Format(_("Supports layers '{0}'"))
                  .arg(mrv::app::soporta_layers);
        LOG_INFO(msg);
        msg = tl::string::Format(_("Supports python '{0}'"))
                  .arg(mrv::app::soporta_python);
        LOG_INFO(msg);
        msg = tl::string::Format(_("Supports voice annotations '{0}'"))
                  .arg(mrv::app::soporta_voice);
        LOG_INFO(msg);
    }
...
namespace mrv
{
    ...
    std::vector<std::string> get_machine_ids()
    {
        ...
        {
            mrv::os::exec_command(
                "powershell \"Get-CimInstance -ClassName "
                "Win32_ComputerSystemProduct | Select-Object -ExpandProperty "
                "UUID\"",
                output, errors);
                ...
                KEY_READ, &hKey) == ERROR_SUCCESS)
        {
            ...
            if (RegGetValueA(
                    hKey, nullptr, "MachineGuid", RRF_RT_REG_SZ, nullptr,
    ...
    void get_network_configuration(
        std::string& server, int& port, std::vector<std::string>& machine_ids,
        std::string& master_key)
    {
        ...
        {
            std::string msg =
                string::Format(_("Found floating license at '{0}'"))
                    .arg(license_file);
            LOG_INFO(msg);
        ...
        else
        {
            ...
            {
                std::string msg =
                    string::Format(_("Did not find floating license at '{0}'"))
                        .arg(path);
                LOG_STATUS(msg);
            }
        }
     ...
     */
    nlohmann::json post_request(
        const std::string serverHost, const int serverPort,
        const std::string& entryPoint, const std::string& requestBody,
        const bool has_master_key = false)
    {
        ...
        try
        {
            ...
            if (!file::isReadable(caLocation))
            {
                ...
                const std::string msg =
                    string::Format(
                        _("{0} is not readable. Using system default."))
                        .arg(caLocation);
                LOG_STATUS(msg);
#    ifdef __linux__
...
#    endif
            ...
            HTTPRequest request(
                HTTPRequest::HTTP_POST, entryPoint, HTTPMessage::HTTP_1_1);
            ...
            {
                LOG_ERROR(
                    "Server is down or your internet connection is failing.");
            ...
            {
                if (entryPoint == "/node_locked_license" && has_master_key)
                    return "";
        ...
        catch (const Exception& ex)
        {
            ...
            if (dynamic_cast<const ConnectionRefusedException*>(&ex) ||
                dynamic_cast<const HostNotFoundException*>(&ex) ||
                dynamic_cast<const DNSException*>(&ex) ||
                std::string(ex.displayText())
                        .find("Name or service not known") !=
                    std::string::npos ||
                std::string(ex.displayText()).find("NXDOMAIN") !=
                    std::string::npos)
            {
                std::string msg = "DNS resolution failed – very likely blocked "
                                  "by Pi-hole/AdGuard/hagezi Light";
                LOG_ERROR(msg);
            }
    ...
    bool send_heartbeat()
    {
        ...
        get_network_configuration(
            serverHost, serverPort, machine_ids, master_key);
        ...
        nlohmann::json json_data =
            post_request(serverHost, serverPort, "/heartbeat", requestBody);
    ...
    License validate_floating(std::string& expiration_date)
    {
        ...
        nlohmann::ordered_json json_data = post_request(
            serverHost, serverPort, "/checkout_license", requestBody);
    ...
    std::string request_webrtc_ticket_floating()
    {
        ...
        // --- HTTP POST to /checkout_license ---
        nlohmann::ordered_json json_data = post_request(
            serverHost, serverPort, "/webrtc_ticket_floating", requestBody);
        if (json_data.is_null() || !json_data.contains("signature"))
        {
            // The error message was already logged inside post_request.
            // We just need to stop here.
    ...
    std::string request_webrtc_ticket_node_locked()
    {
        ...
        for (const auto& id : machine_ids)
        {
            ...
            const std::string requestBody = "{\"machine_id\":\"" + id +
                                            "\",\"plan\":\"" + requestVersion +
                                            "\"}";

            // --- HTTP POST to /webrtc_ticket ---
            nlohmann::json json_data = post_request(
                serverHost, serverPort, "/webrtc_ticket", requestBody);
        ...
        std::string signature =
            valid_json_data.at("signature").get<std::string>();
        const nlohmann::ordered_json& payload_json =
            valid_json_data.at("payload");
        ...
        const std::string expires_at =
            payload_json.at("expires_at").get<std::string>();
        const std::string payload_machine_id =
            payload_json.at("machine_id").get<std::string>();
        const std::string purpose =
            payload_json.at("purpose").get<std::string>();
        ...
        {
            LOG_ERROR(
                "WebRTC ticket mismatch (machine_id or purpose invalid).");
            return "";
        }
    ...
    License validate_node_locked(std::string& expiration_date)
    {
        ...
        for (const auto& id : machine_ids)
        {
            ...
            json_data = post_request(
                serverHost, serverPort, "/node_locked_license", requestBody,
                !master_key.empty());
            if (json_data.is_null() || !json_data.contains("signature") ||
                !json_data.contains("payload"))
                continue;
        ...
        const std::string msg =
            string::Format(_("Valid license for {0}")).arg(machine_id);
        LOG_STATUS(msg);
    ...
    bool release_license()
    {
        ...
        // --- HTTP POST to /checkout_license ---
        nlohmann::ordered_json json_data = post_request(
            serverHost, serverPort, "/release_license", requestBody);
        if (json_data.is_null() || !json_data.contains("status"))
        {
    ...
    License validate_license(std::string& expiration_date)
    {
        ...
        if (app::license_type != LicenseType::kDemo && out == License::kValid)
        {
            /* xgettext:c++-format */
            std::string msg =
